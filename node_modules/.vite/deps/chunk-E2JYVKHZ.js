...
var win3 = typeof window == "object";
var THREE3 = win3 && window.THREE;
...
var getNewBirdGeometry = (options) => {
  const scope = new THREE3.BufferGeometry();
  if (options.quantity) {
  ...
  const vertices = new THREE3.BufferAttribute(new Float32Array(points * 3), 3);
  const birdColors = new THREE3.BufferAttribute(new Float32Array(points * 3), 3);
  const references = new THREE3.BufferAttribute(new Float32Array(points * 2), 2);
  const birdVertex = new THREE3.BufferAttribute(new Float32Array(points), 1);
  if (!scope.setAttribute)
    scope.setAttribute = scope.addAttribute;
...
var getNewBirdGeometryBasic = (options = {}) => {
  ...
  function v(x, y, z) {
    ...
    points.push(new THREE3.Vector3(x * s, y * s, z * s));
  }
  v(5, 0, 0);
  v(-5, -1, 1);
  v(-5, 0, 0);
...
var Boid = class {
  constructor(options) {
    var vector = new THREE3.Vector3(), _acceleration, _width = 500, _height = 500, _depth = 200, _goal, _neighborhoodRadius = 100, _maxSpeed = 2.5, _maxSteerForce = 0.1, _avoidWalls = true;
    var _options = options;
    this.position = new THREE3.Vector3();
    this.velocity = new THREE3.Vector3();
    _acceleration = new THREE3.Vector3();
    ...
    this.alignment = function(boids) {
      var boid, velSum = new THREE3.Vector3(), count = 0, distance;
    ...
    this.cohesion = function(boids) {
      var boid, distance, posSum = new THREE3.Vector3(), steer = new THREE3.Vector3(), count = 0;
    ...
    this.separation = function(boids) {
      var boid, distance, posSum = new THREE3.Vector3(), repulse = new THREE3.Vector3();
...
var Birds = class extends base_default {
  ...
  constructor(userOptions) {
    THREE3 = userOptions.THREE || THREE3;
    super(userOptions);
  }
  initComputeRenderer() {
    this.gpuCompute = new GPUComputationRenderer_default(WIDTH, WIDTH, this.renderer, THREE3);
    ...
    this.velocityUniforms.predator = { value: new THREE3.Vector3() };
    this.velocityVariable.material.defines.BOUNDS = BOUNDS.toFixed(2);
    this.velocityVariable.wrapS = THREE3.RepeatWrapping;
    this.velocityVariable.wrapT = THREE3.RepeatWrapping;
    this.positionVariable.wrapS = THREE3.RepeatWrapping;
    this.positionVariable.wrapT = THREE3.RepeatWrapping;
    const error2 = this.gpuCompute.init();
    if (error2 !== null) {
  ...
  initGpgpuBirds() {
    ...
    this.birdUniforms = {
      color: { value: new THREE3.Color(16720384) },
    ...
    const birdMesh = new THREE3.Mesh(geometry, material);
  ...
  getNewCol(order) {
    ...
    const c1 = new THREE3.Color(color1);
    const c2 = new THREE3.Color(color2);
    const gradient = options.colorMode.indexOf("Gradient") != -1;
    let c, dist;
    if (gradient) {
      dist = Math.random();
    ...
    if (options.colorMode.indexOf("variance") == 0) {
      ...
      c = new THREE3.Color(r2, g2, b2);
    } else if (options.colorMode.indexOf("mix") == 0) {
      c = new THREE3.Color(color1 + dist * color2);
    } else {
      c = c1.lerp(c2, dist);
    }
    return c;
  ...
  onInit() {
    this.camera = new THREE3.PerspectiveCamera(75, this.width / this.height, 1, 3e3);
    this.camera.position.z = 350;
    this.fog = new THREE3.Fog(16777215, 100, 1e3);
    ...
    if (GPGPU) {
      ...
      for (var i = 0; i < numBirds; i++) {
        ...
        const birdColors = new THREE3.BufferAttribute(new Float32Array(numV), 3);
        if (gradient) {
        ...
        bird = birds[i] = new THREE3.Mesh(
          newBirdGeo,
          new THREE3.MeshBasicMaterial({
            color: 16777215,
            side: THREE3.DoubleSide,
            // colors: THREE.VertexColors,
            vertexColors: THREE3.VertexColors
          })
        );
  ...
  onMouseMove(x, y) {
    ...
    if (!GPGPU) {
      const boids = this.boids;
      let boid;
      var vector = new THREE3.Vector3(this.mouseX * this.width, -this.mouseY * this.height, 0);
      for (var i = 0, il = boids.length; i < il; i++) {
        boid = boids[i];
...
//# sourceMappingURL=chunk-E2JYVKHZ.js.map
